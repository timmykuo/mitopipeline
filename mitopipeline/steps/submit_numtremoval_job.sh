...
echo '#SBATCH -J align_and_numt_removal_'"$1" >> $FILE
echo '#SBATCH -A txl80' >> $FILE

echo 'bash /mnt/rds/txl80/LaframboiseLab/tyk3/scripts/align_and_NuMT_removal_fixgaps.sh '"$1"' >> /mnt/rds/txl80/LaframboiseLab/tyk3/TARGET_PIPELINE/STDOUT/'"$1"'_align_and_numt_removal.out 2>&1' >> $FILE
sleep 1
fi
...
#run slurm job
N=4
batchId=`sbatch /mnt/rds/txl80/LaframboiseLab/tyk3/scripts/slurm/align_and_numt_removal_$1.slurm | awk -v N=$N '{print $N}'`
...
rm align_and_numt_removal_$1.slurm
