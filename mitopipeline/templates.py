...
task_with_req_template = Template(
    "class {{ task_name }}(luigi.Task):\n    id=luigi.Parameter()\n    def require(self):\n        return {{ req_name }}(id=self.id)\n    def run(self):\n        subprocess.call([STEPS + \'/{{ job_name }}\', self.id_{{ prev_step }}, PIPELINE_START + \'/{{ req_name }}\', PIPELINE_STORE + \'/{{ task_name }}\', TOOLS, STEPS, REFS])\n    def output(self):\n        return luigi.LocalTarget(PIPELINE_STORE + \'/{{ task_name }}/\' + \'{}_{{ file_name }}\'.format(self.id))")

#slurm template for task with a required task
slurm_task_with_req_template = Template(
    "class {{ task_name }}(luigi.Task):\n    id=luigi.Parameter()\n    def require(self):\n        return {{ req_name }}(id=self.id)\n    def run(self):\n        subprocess.call([STEPS + \'/submit_job.sh\', self.id_{{ prev_step }}, {{ job_name }}, SLURM_DIR, PIPELINE_START + \'/{{ req_name }}\', PIPELINE_STORE + \'/{{ task_name }}\', TOOLS, STEPS, REFS])\n    def output(self):\n        return luigi.LocalTarget(PIPELINE_STORE + \'/{{ task_name }}/\' + \'{}_{{ file_name }}\'.format(self.id))")
